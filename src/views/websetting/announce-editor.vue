  ...
  <div class="frame-page">
    <div class="h-panel h-panel-no-border shadow">
      <div class="h-panel-bar">
        <div class="h-panel-title">网站公告信息修改</div>
      </div>
      <div class="h-panel-bar">
        <Row :space="20">
...
export default {
  ...
  methods: {
    submitSearch() {
      ...
      if (validResult.result) {
        ...
        R.Announce.editorAnnounce(this.announce).then(res => {
          console.log(res);
          if (res.ok) {
            if (res.ok) {
              this.$Notice({
  ...
  mounted() {
    this.seteditor();
    this.announceId = this.$route.params.id;
    console.log(this.announceId);
    if (this.announceId == null) return;

    R.Announce.getOne(this.announceId).then(res => {
      console.log(res);
      if (res.ok) {
        this.announce = res.body;
        this.info_ = this.announce.content;
