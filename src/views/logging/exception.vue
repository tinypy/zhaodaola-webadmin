<template>
  <div class="frame-page">
      ...
      <div class="h-panel-body bottom-line">
        ...
        <Table :datas="datas" ref="logTable" :loading="loading" :border="border">
          ...
          <template slot="expand" slot-scope="{index, data}">
            ...
            <Button color="primary" @click="showError(data)">查看异常详情</Button>
          </template>
        </Table>
      </div>
      <div class="h-panel-bar">
    ...
    <el-dialog title="异常信息" :visible.sync="dialogVisible" :modal-append-to-body="false" width="60%">
...
export default {
  ...
  methods: {
    exportData() {
      ...
      R.Log.download(this.search, 'ERROR').then(res => {
        if (res.ok) {
          Utils.downloadFile(res.body, '异常日志数据');
        }
    ...
    submitSearch() {
      ...
      R.Log.getLogList(this.search, 'ERROR').then(res => {
        console.log(res);
        if (res.ok) {
